...
import mashup.ggiriggiri.gifticonstorm.domain.sprinkle.vo.*
...
class SprinkleRepositoryCustomImpl(
    ...
    override fun findAllByDeadLine(leftMinute: Long, limit: Long): List<GetSprinkleVo> {
        return jpaQueryFactory
            .select(QGetSprinkleVo(
                sprinkle.id,
                coupon.brandName,
            ...
            .fetch().stream().sorted(
                Comparator.comparingInt(GetSprinkleVo::participants).reversed()
            ).limit(limit).collect(Collectors.toList())
    }

    override fun findAllByCategory(category: Category, noOffsetRequest: NoOffsetRequest): List<GetSprinkleVo> {
        return jpaQueryFactory
            .select(QGetSprinkleVo(
                sprinkle.id,
                coupon.brandName,
